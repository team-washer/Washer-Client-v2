...
```powershell
powershell "${CLAUDE_SKILL_DIR}/scripts/get-pr-data.ps1"
```
...
```powershell
find .antigravity/rules -name "*.md" 2>/dev/null
```

Rule priority:

```text
AGENTS.md > ANTIGRAVITY.md > .antigravity/rules/** > nearby source patterns
```
...
```powershell
powershell "${CLAUDE_SKILL_DIR}/scripts/reply-review-comment.ps1" "<repo>" "<pr_number>" "<comment_id>" "<reply_body>"
```
