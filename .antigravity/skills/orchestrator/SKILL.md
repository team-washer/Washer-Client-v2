...
| ---------------------------- | ------ | ------ | --------------------------------- | ----------------------------------- | ---------------------------------------------------------- |
| `frontend-architect`         | custom | gemini-3.5-pro | FSD planning and scope control    | `frontend-architecture`             | `_workspace/{phase}_frontend-architect_plan.md`            |
| `ui-implementation-engineer` | custom | gemini-3.5-pro | React and Tailwind implementation | `ui-implementation`                 | `_workspace/{phase}_ui-implementation-engineer_changes.md` |
| `api-data-integrator`        | custom | gemini-3.5-pro | API, hooks, schemas, and types    | `api-data-flow`                     | `_workspace/{phase}_api-data-integrator_data-flow.md`      |
| `qa-inspector`               | custom | gemini-3.5-pro | Cross-boundary verification       | `quality-gate`                      | `_workspace/{phase}_qa-inspector_report.md`                |
| `code-reviewer`              | custom | gemini-3.5-pro | Changed-file review               | `code-review`                       | `_workspace/{phase}_code-reviewer_review.md`               |
| `bug-investigator`           | custom | gemini-3.5-pro | Root-cause debugging              | `systematic-debugging`              | `_workspace/{phase}_bug-investigator_report.md`            |
| `git-workflow-assistant`     | custom | gemini-3.5-flash  | Commit, PR, and review workflow   | `commit` / `write-pr` / `review-pr` | `_workspace/{phase}_git-workflow-assistant_git.md`         |
...
3. Read `ANTIGRAVITY.md`, `AGENTS.md`, `package.json`, dynamically discovered `.antigravity/rules/*.md`, and the relevant source files before assigning work.
...
TeamCreate(
  ...
  members: [
    { name: "frontend-architect", agent_type: "frontend-architect", model: "gemini-3.5-pro" },
    { name: "ui-implementation-engineer", agent_type: "ui-implementation-engineer", model: "gemini-3.5-pro" },
    { name: "api-data-integrator", agent_type: "api-data-integrator", model: "gemini-3.5-pro" },
    { name: "qa-inspector", agent_type: "qa-inspector", model: "gemini-3.5-pro" },
    { name: "code-reviewer", agent_type: "code-reviewer", model: "gemini-3.5-pro" },
    { name: "bug-investigator", agent_type: "bug-investigator", model: "gemini-3.5-pro" },
    { name: "git-workflow-assistant", agent_type: "git-workflow-assistant", model: "gemini-3.5-flash" }
  ]
)
```
...
- `commit/scripts/discover-changed-areas.ps1` for commit and PR scope discovery.
- `commit/references/scope-guide.md` for scope/type selection.
- `write-pr/scripts/create-pr.ps1` for explicit PR creation requests.
- `write-pr/references/labels.md` for PR label selection.
- `review-pr/scripts/get-pr-data.ps1` for explicit PR review comment handling.
- `review-pr/scripts/reply-review-comment.ps1` for explicit GitHub review replies.
